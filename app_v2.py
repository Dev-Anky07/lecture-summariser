...
def get_transcript(audio_url, ftype):
  config = aai.TranscriptionConfig(
      speaker_labels=True,
      speakers_expected=2,
      custom_vocabularies=['term1', 'term2'], # Add custom vocabulary
      filter_profanity=True, # Filter out profanity
      remove_disfluencies=True, # Remove disfluencies like 'um', 'uh', etc.
  )
...
def ask_question(transcript, question):
  params = {
      ...
      'summary_model': aai.SummarizationModel.informative, # Change the summary model
      'summary_type': aai.SummarizationType.bullets_verbose, # Change the summary type
  }
  summary = transcript.lemur.summarize(**params)
  return summary.response.strip().split('\n')
...
if f:
   ...
   if entered:
       ...
       with st.spinner("Generating summary..."):
           try:
               summary = ask_question(st.session_state['transcript'], context)
